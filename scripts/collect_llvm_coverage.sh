...
cmake -DCMAKE_BUILD_TYPE=Coverage -S "${git_root}" -B "${cov_make_dir}" -G Ninja
...
llvm-cov export -format=lcov -instr-profile="${cov_data_dir}/test.profdata" $(find "${cov_make_dir}/tests" -type f -perm -100 -exec echo -n \ -object {} \;) > "${cov_data_dir}/lcov-all.info"

# Filter the code coverage
lcov -r "${cov_data_dir}/lcov-all.info" '*/extern/*' '*/*_test' -o "${cov_data_dir}/lcov.info" --config-file "${lcovrc}"
